  ...
  public SuccessFlag consume() {
    ...
    try {
      ...
      try {
        ...
        successFlag = new SuccessFlag(false, null,"process exception: " + e);
        e.printStackTrace();
      }
      // 确认消息发送成功,就返回ACK
      ...
      e.printStackTrace();
      return new SuccessFlag(false,null, "interrupted exception " + e.toString());
    } catch (ShutdownSignalException | ConsumerCancelledException | IOException e) {
      e.printStackTrace();
      closeConnection(channel);
      // 如果走到这,则表示最初的连接未成功,或者是中途断开了,因此重新连接.
      consumer = buildQueueConsumer(connection, queue);
      return new SuccessFlag(false,null, "shutdown or cancelled exception " + e.toString());
    } catch (Exception e) {
      e.printStackTrace();
      ...
      // 如果走到这,则表示最初的连接未成功,或者是中途断开了,因此重新连接.
      consumer = buildQueueConsumer(connection, queue);
      return new SuccessFlag(false, null,"exception " + e.toString());
    }
  }
